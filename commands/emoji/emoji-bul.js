...
if(!client.emojis.cache.find(x => x.name === args[0]) && client.emojis.cache.find(x => x.name.endsWith(args[0])) && !client.emojis.cache.find(x => x.name.startsWith(args[0]))) return message.channel.send(new Discord.MessageEmbed().setTitle("<a:not_found:767708663141105664> Uyarı").setColor("BLUE").setDescription('Aradığınız isme tam olarak uyan veya benzeyen bir emoji bulamadım.'));
